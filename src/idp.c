...
int main(int argc, char *argv[]) {
	...
	int bindReturn = bind(socketServer, (struct sockaddr *)&serverAddress,
	...
	// Receive data
	int receivedBytes =
	    recvfrom(socketServer, sendReceiveBuffer, MAX_MESSAGE_SIZE, 0,
		     (struct sockaddr *)&receiveAddress, &addressLength);
	...
	cert_sign(idpID, nodeID, &hc1, seq);
	...
	int sentBytes =
	    sendto(socketServer, msg, strlen(msg), 0,
		   (struct sockaddr *)&receiveAddress, addressLength);
	...
	assert_sign(idpID, nodeID, &hc2, seq);

	buildMessage(&hc2, msg2);

	// Reply the received message to the client
	sentBytes = -1;
	sentBytes = sendto(socketServer, msg2, strlen(msg2), 0,
			   (struct sockaddr *)&receiveAddress, addressLength);
